...
import type { AccountsByPlatform, SelectedAccountsByPlatform } from "../types/createPostTypes";
...
export const PlatformSelector: React.FC<PlatformSelectorProps> = ({
  ...
  const dropdownRefs = React.useRef<{ [platformId: string]: HTMLDivElement | null }>({});
    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {PLATFORMS.map((platform) => {
          ...
          const selectedAccountIds = selectedAccountsByPlatform[platform.id] || [];
          ...
          return (
            <div key={platform.id} className="relative">
              ...
              {openDropdown === platform.id && linkedAccounts.length > 0 && (
                <div
                  ref={el => { dropdownRefs.current[platform.id] = el; }}
                  ...
                  style={{ minWidth: '14rem', boxShadow: '0 8px 32px 0 rgba(0,0,0,0.18)' }}
                >
                  ...
                  {linkedAccounts.map((account, idx) => (
                    <div
                      ...
                      onClick={e => {
                        ...
                        if (!selectedPlatforms.includes(platform.id) && onTogglePlatform) {
                          onTogglePlatform(platform.id);
                        }
                      }}
                    >
                      {account.displayName} <span className="ml-2 text-gray-400">{account.username}</span>
                    </div>
                  ))}
                </div>
              )}
